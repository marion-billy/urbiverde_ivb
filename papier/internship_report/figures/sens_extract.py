...
# Perturbed runs are consolidated in one long CSV (perturbation, city, guild + stat cols).
_ALL = pd.read_csv(os.path.join(ROOT, "_sandbox", "sensitivity", "all_stats.csv"))


def val(city, guild, metric, tag=None):
    if tag is None:                                             # baseline stays in data/outputs
        f = glob.glob(f"{ROOT}/data/outputs/{city}/{guild}/stats_*.csv")
        if not f:
            return None
        s = pd.read_csv(f[0]).iloc[0]
    else:
        row = _ALL[(_ALL["perturbation"] == tag) & (_ALL["city"] == city) & (_ALL["guild"] == guild)]
        if row.empty:
            return None
        s = row.iloc[0]
