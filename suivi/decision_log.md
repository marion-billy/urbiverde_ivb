...
## 2026-08-30 (Marion) -- Synchronisation du dépôt Git et étiquette de fin de stage

Le dépôt `github.com/marion-billy/urbiverde_ivb`, ouvert en début de stage, n'avait pas été tenu au
fil du développement : le partage quotidien passait par l'espace de travail commun de la VM. L'état
livré y est désormais reporté.

Méthode retenue : `git init` **dans** le dossier de travail, `git remote add`, `git fetch`, puis
`git reset --soft origin/<branche>` pour rattacher l'historique distant sans toucher au répertoire de
travail ; `git add -A` filtré par le nouveau `.gitignore`, puis un commit unique. L'alternative par
clone dans un dossier tiers a été écartée : elle imposait de sélectionner les fichiers à recopier, là
où le `.gitignore` fait ce travail de façon vérifiable. Étiquette `v1.0-stage` posée sur ce commit.

Choix assumé sur l'historique : **aucun commit n'est antidaté**. Le dépôt porte un commit unique daté
du jour, dont le message dit explicitement que le versionnage n'a pas été tenu pendant le
développement et renvoie à ce journal pour l'historique des décisions. Le rapport (§2.5, chapitre 6)
décrit cet usage inégal des outils plutôt que de laisser croire à un versionnage continu.

Périmètre : code (`utils/`, `tests/`), configuration des profils, documentation de suivi, sources du
rapport, notebooks dépouillés de leurs sorties (`nbstripout`). Sont exclus `data/` (116 Go), les
caches, la clé de compte de service Earth Engine et les binaires lourds du dossier `papier/`, via un
`.gitignore` dont les exceptions réouvrent les neuf fichiers de documentation des sorties
(`data/outputs/README*.md`, `species_params.csv`, `pipeline.*`, synthèses de sensibilité). Un contrôle
sur l'index avant commit (recherche de `.tif`, `.geojson`, `credential`, `gee*.json`) n'a rien
signalé. `[Certain]`

Deux points pratiques rencontrés, notés dans `suivi/reproductibilite.md` §1.1 : un `git commit -m`
multiligne laisse le shell en attente du guillemet fermant et avale les commandes suivantes dans le
message (utiliser `git commit -F -` avec un document en ligne) ; et GitHub n'accepte plus
l'authentification par mot de passe, un jeton personnel est nécessaire.

Conséquence pour la suite : la copie de la VM devient la référence. Toute modification du code ou de
la documentation de suivi doit y être faite puis commitée, sous peine de divergence avec les copies
de travail locales.

## 2026-08-30 (Marion) -- Le cache d'occupation du sol n'a jamais été écrit, sauf pour Kourou

Découvert en constituant les caches manquants. `lc_osm.to_parquet()` échoue avec
`ArrowTypeError: Expected bytes, got a 'int' object -- Conversion failed for column width`.

Cause. OpenStreetMap expose un tag `width`, donc `osm_proj` porte déjà une colonne `width` de
chaînes. `landcover.download_lc_data` (l. 146) n'y écrit des largeurs entières que pour les voies ;
les surfaces en eau, le bâti et les emprises artificialisées conservent la valeur textuelle du tag.
Après le `pd.concat`, la colonne mélange chaînes et entiers, type qu'Arrow ne sait pas inférer.

Conséquence, plus lourde que le symptôme. `run_pipeline.py` (l. 256) attrape cette exception et se
contente d'un message avant de poursuivre : l'échec est passé inaperçu. `--lc-cache` n'a donc
**jamais rien écrit**, sauf pour Kourou dont l'extraction OSM ne porte apparemment aucun tag
`width`. Chaque exécution dite « avec cache » repartait en réalité d'un téléchargement Earth Engine
et d'une requête Overpass. Cela explique le dossier `Perpignan_6000` réduit à son seul raster, et
relativise la vérification `_sandbox/verify_lc_cache.sh` du 2026-08-03 : sur Perpignan, elle
comparait deux téléchargements et non un cache à un téléchargement.

Ce que cela ne remet pas en cause. Le test de déterminisme du jour reste valide : à défaut de cache,
les deux exécutions ont téléchargé, la seconde étant servie par le cache HTTP d'OSMnx
(`data/cache/`), donc à partir des mêmes données. C'est d'ailleurs ce que l'identité octet par octet
démontre indépendamment.

Correctif appliqué. `utils/build_lc_cache.py` tente l'écriture native puis, en cas d'échec,
convertit en chaîne les colonnes de tags en cause, en préservant les seules colonnes lues en aval
(`geometry`, `wc_code`, `wc_order`, vérifiées par relecture de landcover, connectivity et
run_pipeline). Le parquet est désormais écrit avant le raster, pour ne jamais laisser un cache à
moitié constitué. `[Certain]`

Reste à faire, hors périmètre du dépôt : corriger la cause dans `landcover.download_lc_data` (ne pas
réutiliser le nom `width`, ou ne retourner que les colonnes utiles), et faire remonter l'échec
d'écriture du cache dans `run_pipeline.py` au lieu de l'avaler.

## 2026-08-30 (Marion) -- Remise en service du cache d'occupation du sol : trois défauts, un contrôle croisé

Constitution des archives d'entrée manquantes (`data/lc_cache/<Ville>_<tampon>/`). Trois défauts se
masquaient les uns les autres, chacun révélé par la correction du précédent.

1. **L'écriture échouait silencieusement.** `lc_osm.to_parquet()` levait
   `ArrowTypeError` sur la colonne `width` : OpenStreetMap expose un tag du même nom, si bien que la
   colonne mélange les chaînes des tags et les largeurs entières calculées pour les voies.
   `run_pipeline.py` (l. 256) attrapait l'exception et poursuivait. Vérifié sur 696 journaux
   d'exécution : aucun ne porte « Land-cover from cache ». `--lc-cache` n'avait donc **jamais** rien
   lu ni écrit, et aucun résultat publié n'a été produit à partir d'un cache.

2. **Le raster était divisé par dix.** Earth Engine attache un facteur d'échelle à la bande ;
   `rio.to_raster` le respecte et dé-scale à l'écriture, `open_rasterio` ne le ré-applique pas à la
   relecture. Le cache portait les valeurs 1, 2, 3 au lieu des codes 10, 20, 30 : une exécution sur
   cache trouvait zéro tache d'habitat sur les quatre profils.

3. **La priorité de brûlage était perdue.** `wc_order` est une catégorie ORDONNÉE
   (`[50, 80, 51, 52, 53, 55, 54]`) que le parquet dégrade en entiers. Le tri redevenait numérique
   (50, 51, ... 80), plaçant l'eau en priorité maximale : elle recouvrait les ponts, rendant les
   cours d'eau infranchissables pour les profils dont l'eau est une barrière. Sur Perpignan, le
   petit mammifère terrestre passait de 7 à 55 liens en échec et d'un à deux sous-réseaux, quand les
   profils arboricole et de lisière, pour qui l'eau n'est pas une barrière, ne bougeaient que de
   2 à 3 %. La surface d'habitat, elle, ne variait que de 0,1 % : le défaut était invisible dans les
   grandeurs habituellement surveillées.

Correctifs, tous dans `utils/build_lc_cache.py`, avec vérification par relecture immédiate : le rang
de brûlage est écrit en entier explicite plutôt qu'en catégorie, le raster est écrit en `uint8` sans
facteur d'échelle, et chaque écriture est relue et contrôlée avant que le cache soit déclaré valide.
Un cache faux échoue désormais franchement au lieu de produire des sorties plausibles.

**Contrôle retenu.** Une exécution partant de l'archive et une exécution partant d'un téléchargement
direct, le même jour et avec le même code, donnent des indicateurs identiques sur les quatre profils
de Perpignan. `[Certain]` Ce contrôle confronte deux chemins de calcul distincts censés converger,
là où le test de déterminisme rejoue deux fois le même : c'est lui qui a débusqué le troisième
défaut, qu'aucune répétition n'aurait pu voir. À conserver comme contrôle de reproductibilité à part
entière.

**Portée.** L'archive fige l'état d'OpenStreetMap au 2026-08-30, non celui du 2026-07-06 des jeux de
référence, qui n'a pas été conservé. Une exécution depuis l'archive reproduit donc fidèlement la
chaîne sans redonner exactement les chiffres publiés : sur Perpignan, l'écart est de 0,1 % en surface
d'habitat et plus marqué sur la topologie, le tissu cartographié dans OSM s'étant densifié depuis
mai. Comportement attendu d'une chaîne assise sur une base vivante, et justification même de
l'archivage pour les exécutions futures.

**Point de méthode relevé au passage.** La colonne `connectivity_loss_pct` a disparu des `stats.csv`
entre les jeux de référence et aujourd'hui, sans trace au journal ; `connectivity.py` et
`landcover.py` ont par ailleurs été modifiés après le 2026-07-06. Une comparaison aux jeux de
référence ne peut donc pas isoler l'effet d'une seule variable, ce qui a d'abord égaré le diagnostic.
Tout changement touchant le schéma des sorties mérite une entrée ici.

Reste à corriger, hors périmètre du dépôt : la cause dans `landcover.download_lc_data` (ne pas
réutiliser le nom `width`), et la remontée de l'échec d'écriture du cache dans `run_pipeline.py`,
aujourd'hui avalé. Le pipeline se termine par ailleurs avec un code de sortie nul même lorsque tous
les profils échouent, ce qu'un ordonnanceur ne verrait pas.
