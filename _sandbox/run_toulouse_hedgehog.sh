#!/bin/bash
# SOLO sweep for Toulouse ground_mammal (herisson, d0=3000, the heaviest profile on the largest AOI).
# One city at a time (full resources), timeout raised to 6h/run, single attempt (a true timeout is not
# rescued by a retry), idempotent (skips tags already computed). Detached: survives disconnection.
#   setsid nohup bash _sandbox/run_toulouse_hedgehog.sh </dev/null >> _sandbox/logs/nohup_th.out 2>&1 &
export PYTHONPATH=/opt/conda/lib/python3.11/site-packages
cd /home/jovyan/work/team/marion/corridor_project || exit 1
C=Toulouse; G=ground_mammal
